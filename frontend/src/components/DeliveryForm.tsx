...
const DeliveryForm: React.FC<DeliveryFormProps> = ({ delivery, onClose, onSave }) => {
  ...
  const onSubmit = async (data: DeliveryFormData) => {
    ...
    try {
      ...
      const cleanData = {
        ...data,
        customerObservations: data.customerObservations || null,
        pickupItems: data.hasPickup ? (data.pickupItems || null) : null,
        deliveredMaterials: data.deliveredMaterials || null,
        paymentAmount: data.paymentAmount || null
      };

      if (delivery) {
        await axios.put(`/api/deliveries/${delivery.id}`, cleanData);
      } else {
        await axios.post('/api/deliveries', cleanData);
      }
      onSave();
    } catch (error: any) {
      ...
      if (error.response?.data?.details) {
        ...
        alert('Error al guardar la entrega. Por favor, verifica los datos.');
      }
    } finally {
      setLoading(false);
    }
