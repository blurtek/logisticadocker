...
const Deliveries: React.FC = () => {
  ...
  const handleComplete = async (id: string) => {
    if (window.confirm('¿Marcar esta entrega como completada y cobrada?')) {
      try {
        await axios.patch(`/api/deliveries/${id}/complete`);
        fetchDeliveries();
        alert('Entrega marcada como completada y cobrada');
      } catch (error) {
        console.error('Error completing delivery:', error);
        alert('Error al marcar la entrega como completada');
      }
    }
  };
                ...
                <div className="flex space-x-2 flex-shrink-0">
                  {delivery.status !== 'COMPLETADO' && (
                    <button 
                      ...
                      title="Marcar como completada y cobrada"
                    >
